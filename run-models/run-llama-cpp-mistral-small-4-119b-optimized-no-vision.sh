#!/usr/bin/env bash
# run-llama-cpp-mistral-small-4-119b-optimized-no-vision.sh
# ----------------------------------------------------------
# Throughput-oriented non-vision preset for Mistral Small 4 119B IQ4_XS.
# Matches the current tuned non-vision placement.
...
cmd=(
  "${LLAMA_SERVER}"
  -m "${MODEL}"
  --alias "unsloth/Mistral-Small-4-119B-2603 (optimized no-vision)"
  --fit off
  -ngl "${N_GPU_LAYERS}"
