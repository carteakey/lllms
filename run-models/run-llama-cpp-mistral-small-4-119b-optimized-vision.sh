#!/usr/bin/env bash
# run-llama-cpp-mistral-small-4-119b-optimized-vision.sh
# -------------------------------------------------------
# Throughput-oriented vision preset for Mistral Small 4 119B IQ4_XS.
# Compared to non-vision, defaults to one fewer GPU layer to leave headroom
# for mmproj + image path.
...
# One layer below non-vision default to keep VRAM headroom for multimodal path.
...
if [ ! -f "${MMPROJ}" ]; then
  echo "vision preset requires mmproj, not found: ${MMPROJ}" >&2
  exit 1
fi
...
cmd=(
  ...
  -m "${MODEL}"
  --mmproj "${MMPROJ}"
  --alias "unsloth/Mistral-Small-4-119B-2603 (optimized vision)"
  --fit off
  -ngl "${N_GPU_LAYERS}"
