...
- Or, for a local Llama server (e.g., running via `./run-llama-cpp-mistral-small-4-119b-optimized-no-vision-thinking.sh`):
   ```python
   [[providers]]
   name = "localllama" # the name of the provider for the Vibe config
   api_base = "http://localhost:8001" # base url of the api (matches the script's default port)
   api_key_env_var = "" # leave empty as local server typically doesn't require API key
   api_style = "openai"
   backend = "generic"
   ```
   ```python
   [[models]]
   name = "unsloth/Mistral-Small-4-119B-2603-GGUF" # id name of the model in the api (check your server's model name)
   provider = "localllama" # name of the provider
   alias = "mistral-local" # the alias for Vibe to use the model
   temperature = 0.7 # adjusted as requested
   top_p = 0.95 # matches the script's default top-p
   top_k = 40 # matches the script's default top-k
   min_p = 0.0 # matches the script's default min-p
   repeat_penalty = 1.0 # matches the script's default repeat penalty
   input_price = 0.0
   output_price = 0.0
   ```
   Then set the active_model to the alias:
   ```python
   active_model = "mistral-local"
   ```
   
   :::note
   Before using this configuration, you need to start the local server using the provided script:
   ```bash
   ./run-llama-cpp-mistral-small-4-119b-optimized-no-vision-thinking.sh
   ```
   The script configures the server with specific parameters optimized for Mistral Small 4 119B, including:
   - Context size: 65536 tokens (64k) - adjusted as requested
   - GPU layers: 9
   - Temperature: 0.7 - adjusted as requested
...
_server_r: done request: POST /v1/chat/completions 127.0.0.1:8001