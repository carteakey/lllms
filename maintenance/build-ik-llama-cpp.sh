#!/bin/bash
#
# build-ik-llama-cpp.sh
# ---------------------
# Builds ikawrakow/ik_llama.cpp on Ubuntu/Arch Linux with CUDA support.
#
...
# ---------------------------------------------------------------------------
# Clone & build ikawrakow/ik_llama.cpp
# ---------------------------------------------------------------------------

LLAMA_REPO="$SCRIPT_DIR/vendor/ik_llama.cpp"
LLAMA_BUILD="$LLAMA_REPO/build"

if [ ! -d "$LLAMA_REPO" ]; then
    log "Cloning ik_llama.cpp into $LLAMA_REPO"
    git clone https://github.com/ikawrakow/ik_llama.cpp "$LLAMA_REPO"
else
    log "Updating existing ik_llama.cpp in $LLAMA_REPO"
...
if [ -n "$CUDA_ARCH" ]; then
    log "Configuring ik_llama.cpp with CUDA support (architecture: $CUDA_ARCH)..."
    CMAKE_ARGS+=(
    "-DGGML_CUDA=ON"
    "-DGGML_NATIVE=ON"
    "-DGGML_LTO=ON"
    ...
    log "Configuring ik_llama.cpp for CPU-only build..."
fi

log "Generating build configuration..."
cmake .. "${CMAKE_ARGS[@]}"

log "Building ik_llama.cpp tools (Release)..."
cmake --build . --config Release --target llama-server llama-batched-bench llama-cli llama-bench llama-sweep-bench --parallel
...
log ""
log "Done! ik_llama.cpp binaries are in: $LLAMA_BUILD/bin"
if [ -n "$CUDA_ARCH" ]; then
